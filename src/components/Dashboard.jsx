...
function Dashboard({ user }) {
  ...
  // Derived state for query dates to pass to children
  ...
  const getQueryDates = () => { //
    ...
    let sDate, eDate = now; 
    switch (dateRange) { //
      case '24h': sDate = new Date(now.getTime() - 24 * 60 * 60 * 1000); break; //
      case '7d': sDate = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000); break; //
      case '1m': sDate = new Date(now.getFullYear(), now.getMonth() - 1, now.getDate(), 0, 0, 0, 0); break; //
      case '1y': sDate = new Date(now.getFullYear() - 1, now.getMonth(), now.getDate(), 0, 0, 0, 0); break; //
      case 'custom': //
        if (customStartDate && customEndDate) { //
          sDate = new Date(customStartDate); sDate.setHours(0, 0, 0, 0); //
          eDate = new Date(customEndDate); eDate.setHours(23, 59, 59, 999); //
        } else { return { startDate: null, endDate: null }; } //
        break;
      case 'all': default: return { startDate: null, endDate: null }; //
    }
    if (dateRange !== 'custom' && eDate > now) eDate = now; //
    if (dateRange !== 'custom' && sDate > now) { //
        sDate = new Date(now.getTime() - (dateRange === '24h' ? 24*60*60*1000 : 7*24*60*60*1000)); //
    }
    return { //
      startDate: sDate ? sDate.toISOString() : null, //
      endDate: eDate ? eDate.toISOString() : null, //
    };
  };

  // Update queryStartDate and queryEndDate whenever date filters change
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchThreatData = async () => { //
      ...
      try {
        ...
        // Use the state variables queryStartDate and queryEndDate
        const params = new URLSearchParams(); //
        if (queryStartDate) params.append('start_date_str', queryStartDate); //
        if (queryEndDate) params.append('end_date_str', queryEndDate); //
  ...
  }, [user?.token, queryStartDate, queryEndDate]); // Add queryStartDate and queryEndDate to dependency array
    ...
    <div className="min-h-screen bg-[#0E0B16]">
      ...
      <div className="max-w-7xl mx-auto mt-10 p-6 bg-[#161025] shadow-lg rounded-xl">
        ...
        {loading && (ipCount === null && eventCategories.length === 0 && eventCounts.length === 0) &&
            <p className="text-purple-400 mb-4 text-center">Loading dashboard data...</p>
        }
        ...
        <div className="bg-[#0E0B16] p-6 rounded shadow space-y-8"> {/* */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"> {/* */}
            ...
            <div className="bg-gray-100 p-6 rounded-lg shadow-md min-h-[350px] flex flex-col"> {/* */}
              ...
              {!loading && !error && (ipCount === null || domainCount === null || urlCount === null || hashCount === null || emailCount === null || regkeyCount === null) &&
                (parseInt(ipCount || "0") + parseInt(domainCount || "0") + parseInt(urlCount || "0") + parseInt(hashCount || "0") + parseInt(emailCount || "0") + parseInt(regkeyCount || "0") === 0) &&
              ...
              {!loading && !error && ipCount !== null && domainCount !== null && urlCount !== null && hashCount !== null && emailCount !== null && regkeyCount !== null &&
               (parseInt(ipCount) + parseInt(domainCount) + parseInt(urlCount) + parseInt(hashCount) + parseInt(emailCount) + parseInt(regkeyCount) > 0) && (
          ...
          {/* ThreatLevelChart and AttributeCategoriesChart in a new row, each taking half width on large screens */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Pass the derived queryStartDate and queryEndDate to ThreatLevelChart */}
            <ThreatLevelChart startDate={queryStartDate} endDate={queryEndDate} /> {/* */}
