...
function IPThreats() {
  const [allIpData, setAllIpData] = useState([]);
  const [ipData, setIpData] = useState([]);
  const [visibleIpCount, setVisibleIpCount] = useState(50);
  ...
  useEffect(() => {
    const fetchIps = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/threats/ips${queryParams}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setAllIpData(response.data);
        // setIpData(response.data.slice(0, 50)); // Handled by other useEffect
        setMatchedIP(null);
      } catch (err) {
        console.error(err.response ? err.response.data : err.message);
        setError(err.response?.data?.detail || 'Failed to load IP threats. Please try again later.');
        setAllIpData([]); setIpData([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    setVisibleIpCount(50);
    setMatchedIP(null);
  }, [dateRange, customStartDate, customEndDate]);

  const handleSearch = () => {
    const trimmedSearch = searchIP.trim().toLowerCase();
    if (!trimmedSearch) {
        setMatchedIP(null);
        return;
    }
    const found = allIpData.find((item) => item.value.toLowerCase() === trimmedSearch);
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-8 flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4">
            <input
              type="text" placeholder="Search an IP address (e.g., 192.168.1.1)"
              ...
              {matchedIP ? (
                <div>
                  <p className="text-green-400 text-lg flex items-center justify-center space-x-2">
                    ...
                    <strong className="break-all">{matchedIP.value}</strong>
                  </p>
                  {matchedIP.event_info && (
                    <p className="text-gray-300 text-sm mt-1 flex items-center justify-center space-x-1">
                        ...
                        <span>Event: {matchedIP.event_info}</span>
                    </p>
                  )}
                </div>
              ) : (
                <p className="text-red-400 text-lg flex items-center justify-center space-x-2">
                  <span className="text-2xl">😞</span> <span>IP not found in the current filtered list.</span>
                </p>
              )}
            </div>
          )}
          ...
          <div className="bg-gray-800 rounded-2xl shadow-xl p-8 mt-4 border border-gray-700">
            {loading ? (
              ...
              <p className="text-center text-gray-500 text-lg py-10">No IP threats found for the selected period.</p>
            ) : (
            ...
            {ipData.length < allIpData.length && !loading && (
            ...
            {ipData.length === allIpData.length && allIpData.length > 0 && !loading && (
