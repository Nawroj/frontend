...
import { FiSearch, FiLogOut, FiArrowLeft, FiHash, FiAlertTriangle, FiEye, FiInfo, FiCalendar } from 'react-icons/fi';
...
function HashThreats() {
  ...
  useEffect(() => {
    const fetchHashThreats = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/threats/hashes${queryParams}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setAllHashData(response.data);
        // setHashData(response.data.slice(0, 50)); // Handled by other useEffect
        setMatchedHash(null);
      } catch (err) {
        console.error('Failed to fetch hash data:', err.response ? err.response.data : err.message);
        setError(err.response?.data?.detail || 'Failed to load hash threat data. Please try again later.');
        setAllHashData([]); setHashData([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    setVisibleHashCount(50);
    setMatchedHash(null);
  }, [dateRange, customStartDate, customEndDate]);

  const handleSearch = () => {
    const trimmedSearch = searchHash.trim().toLowerCase();
    if (!trimmedSearch) {
        setMatchedHash(null);
        return;
    }
    const found = allHashData.find((item) => item.value.toLowerCase() === trimmedSearch);
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-8 flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4">
            <input
              type="text" placeholder="Search a hash (e.g., d41d8cd98f00b204e9800998ecf8427e)"
              ...
              {matchedHash ? (
                 <div>
                  <p className="text-green-400 text-lg flex items-center justify-center space-x-2">
                    ...
                    <strong className="break-all">{matchedHash.value}</strong>
                  </p>
                  {matchedHash.event_info && (
                     <p className="text-gray-300 text-sm mt-1 flex items-center justify-center space-x-1">
                        ...
                        <span>Event: {matchedHash.event_info}</span>
                    </p>
                  )}
                </div>
              ) : (
                <p className="text-red-400 text-lg flex items-center justify-center space-x-2">
                  <span className="text-2xl">😞</span> <span>Hash not found in the current filtered list.</span>
                </p>
              )}
            </div>
          )}
          ...
          <div className="bg-gray-800 rounded-2xl shadow-xl p-8 mt-4 border border-gray-700">
            {loading ? (
              ...
              <p className="text-center text-gray-500 text-lg py-10">No hash threats found for the selected period.</p>
            ) : (
            ...
            {hashData.length < allHashData.length && !loading && (
            ...
            {hashData.length === allHashData.length && allHashData.length > 0 && !loading && (
