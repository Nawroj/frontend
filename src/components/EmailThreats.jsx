...
import { FiSearch, FiLogOut, FiArrowLeft, FiMail, FiAlertTriangle, FiEye, FiInfo, FiCalendar } from 'react-icons/fi';
...
function EmailThreats() {
  ...
  useEffect(() => {
    const fetchEmails = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/threats/emails${queryParams}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setAllEmailData(response.data);
        // setEmailData(response.data.slice(0, 50)); // Handled by the other useEffect based on visibleEmailCount
        setMatchedEmail(null); 
      } catch (err) {
        console.error(err.response ? err.response.data : err.message);
        setError(err.response?.data?.detail || 'Failed to load email threats. Please try again later.');
        setAllEmailData([]); setEmailData([]);
      } finally {
        setLoading(false);
      }
    ...
    fetchEmails();
  }, [dateRange, customStartDate, customEndDate]); // Token is fetched inside, so not a direct dependency here for re-trigger
  ...
  const handleSearch = () => {
    ...
    const found = allEmailData.find((item) => item.value.toLowerCase() === trimmedSearch);
    setMatchedEmail(found || null);
  };

  const handleSeeMore = () => {
    setVisibleEmailCount((prev) => prev + 50);
  };
              ...
              {matchedEmail ? (
                <div>
                  <p className="text-green-400 text-lg flex items-center justify-center space-x-2">
                    <span className="text-2xl">🎉</span> <span>Email found:</span> <strong className="break-all">{matchedEmail.value}</strong>
                  </p>
                  {matchedEmail.event_info && (
                     <p className="text-gray-300 text-sm mt-1 flex items-center justify-center space-x-1">
                        ...
                        <span>Event: {matchedEmail.event_info}</span>
                    </p>
                  )}
                </div>
              ) : (
                <p className="text-red-400 text-lg flex items-center justify-center space-x-2">
                  <span className="text-2xl">😞</span> <span>Email not found in the current filtered list.</span>
                </p>
              )}
            </div>
          )}
          ...
          <div className="bg-gray-800 rounded-2xl shadow-xl p-8 mt-4 border border-gray-700">
            {loading ? (
              ...
              <p className="text-center text-gray-500 text-lg py-10">
                No email threats found for the selected period.
              </p>
            ) : (
            ...
            {emailData.length < allEmailData.length && !loading && (
            ...
            {emailData.length === allEmailData.length && allEmailData.length > 0 && !loading &&(
