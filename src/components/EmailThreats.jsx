...
import { FiSearch, FiLogOut, FiArrowLeft, FiMail, FiAlertTriangle, FiEye } from 'react-icons/fi'; // Added relevant icons

function EmailThreats() {
  const [emailData, setEmailData] = useState([]);
  const [error, setError] = useState('');
  const [searchEmail, setSearchEmail] = useState('');
  const [matchedEmail, setMatchedEmail] = useState(null);
  const [visibleEmailCount, setVisibleEmailCount] = useState(50);
  ...
  useEffect(() => {
    const fetchEmails = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:8000/threats/emails', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        setAllEmailData(response.data);
        setEmailData(response.data.slice(0, 50));
      } catch (err) {
        console.error(err.message);
        setError('Failed to load email threats. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    // This useEffect ensures that if allEmailData changes for any reason,
    // the visible list updates correctly based on visibleEmailCount.
    setEmailData(allEmailData.slice(0, visibleEmailCount));
  }, [visibleEmailCount, allEmailData]);

  const handleSearch = () => {
    // Perform search on `allEmailData` to ensure all loaded data is checked
    const found = allEmailData.find((item) => item.toLowerCase() === searchEmail.trim().toLowerCase());
    setMatchedEmail(found || null);
  };

  const handleSeeMore = () => {
    const newVisibleCount = visibleEmailCount + 50;
    setVisibleEmailCount(newVisibleCount);
    // Directly update emailData with the new slice for immediate rendering
    setEmailData(allEmailData.slice(0, newVisibleCount));
  };
      ...
      <div className="flex flex-1 justify-center p-8">
        <div className="w-full max-w-5xl">
          <h1 className="text-4xl font-extrabold text-white mb-8 text-center drop-shadow-lg flex items-center justify-center">
            <FiMail className="mr-4 text-purple-400" />
            Email <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-500 ml-3">Threats</span>
          </h1>
          ...
          <div className="mb-8 flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4">
            <input
              type="text"
              placeholder="Search an email address (e.g., example@malicious.com)"
              value={searchEmail}
            ...
            <div className="text-center mb-8 p-4 rounded-xl shadow-md
              ...
              {matchedEmail ? (
                <p className="text-green-400 text-lg flex items-center justify-center space-x-2">
                  <span className="text-2xl">🎉</span> <span>Email found:</span> <strong className="break-all">{matchedEmail}</strong>
                </p>
              ) : (
                <p className="text-red-400 text-lg flex items-center justify-center space-x-2">
                  <span className="text-2xl">😞</span> <span>Email not found in threat list.</span>
                </p>
              )}
            </div>
          )}
          ...
          <div className="bg-gray-800 rounded-2xl shadow-xl p-8 mt-4 border border-gray-700">
            {loading ? (
              <div className="flex justify-center items-center py-10">
                ...
                <p className="ml-4 text-gray-400 text-xl">Loading email addresses...</p>
              </div>
            ) : emailData.length === 0 ? (
              <p className="text-center text-gray-500 text-lg py-10">
                No email threats found at this time.
              </p>
            ) : (
            ...
            {/* See More Button */}
            {emailData.length < allEmailData.length && (
              <div className="text-center mt-8">
                ...
                >
                  ...
                  <span>See More ({allEmailData.length - emailData.length} remaining)</span>
                </button>
              </div>
            )}
            {emailData.length === allEmailData.length && allEmailData.length > 0 && (
